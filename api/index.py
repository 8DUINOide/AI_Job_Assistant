...
from docx_resume_generator import generate_resume_docx, convert_user_profile_to_template_data
...
@app.route('/api/generate-resume-docx', methods=['POST'])
@require_auth
def generate_resume_docx_endpoint():
    """Generates a DOCX resume from the template using user's profile data."""
    try:
        data = request.json
        ...
        docx_buffer = generate_resume_docx(profile_data)
        
        name = profile_data.get('personal_info', {}).get('name', 'Resume')
...
@app.route('/api/generate-resume-template-pdf', methods=['POST'])
@require_auth
def generate_resume_template_pdf():
    """Generates a PDF resume matching the template layout using user's profile data."""
    try:
        data = request.json
        profile_data = data.get('profile_data')
        if not profile_data:
        ...
        # Convert template format to the existing PDF generator format
        ...
        pdf_data = {
            'personal_info': {
                ...
                'phone': pi.get('phone', ''),
                'location': pi.get('location', ''),
                'link': pi.get('linkedin', '') or pi.get('github', '')
            },
            'summary': '',
            'sections': []
        }
        ...
        for edu in profile_data.get('education', []):
            details = []
            if edu.get('details'):
                details.append(edu['details'])
            if edu.get('coursework'):
                details.append(f"Relevant Coursework: {edu['coursework']}")
            edu_items.append({
        ...
        if edu_items:
            pdf_data['sections'].append({'title': 'EDUCATION', 'items': edu_items})
        ...
        for exp in profile_data.get('experience', []):
            ...
            subtitle = f"{company} | {loc}" if loc else company
            exp_items.append({
                'title': exp.get('title', ''),
                'subtitle': subtitle,
        ...
        if exp_items:
            pdf_data['sections'].append({'title': 'WORK EXPERIENCE', 'items': exp_items})
        ...
        if proj_items:
            pdf_data['sections'].append({'title': 'PROJECTS', 'items': proj_items})
        
        # Activities section
        act_items = []
        for act in profile_data.get('activities', []):
            act_items.append({
                'title': act.get('organization', ''),
                'subtitle': act.get('role', ''),
                'date': act.get('date_range', ''),
                'bullets': act.get('bullets', [])
            })
        if act_items:
            pdf_data['sections'].append({'title': 'ACTIVITIES', 'items': act_items})
        
        # Additional section
        add = profile_data.get('additional', {})
        add_items = []
        if add.get('technical_skills'):
            add_items.append({'title': 'Technical Skills', 'subtitle': '', 'date': '', 'bullets': [add['technical_skills']]})
        if add.get('certifications'):
            add_items.append({'title': 'Certifications', 'subtitle': '', 'date': '', 'bullets': [add['certifications']]})
        if add.get('languages'):
            add_items.append({'title': 'Languages', 'subtitle': '', 'date': '', 'bullets': [add['languages']]})
        if add_items:
            pdf_data['sections'].append({'title': 'ADDITIONAL', 'items': add_items})
        
        pdf_buffer = generate_pdf_from_data(pdf_data)
        
        name = pi.get('name', 'Resume')
